...
export const pullRounds: PullRound[] = [
  ...
  { id: "rd_11", vendor_id: "kiroappio", bus_id: "bus_weekend", bus_name: "周末拼车局", result: "refunded", count_requested: 1, count_purchased: 1, alive_count: 0, dead_count: 1, push_state: "none", push_ratio: null, total_cost: C(9), fail_reason: "30 分钟内死亡 · 质保退款", created_at: ago(126) },
...
export const activities: Activity[] = [
  { id: "a1", kind: "extract", summary: "+2 号 · Kiro Drop → 推我的号池", amount: -C(6), created_at: ago(0.5), link: "/extract" },
  { id: "a2", kind: "push", summary: "2 号 → 我的号池 · 成功", amount: null, created_at: ago(0.6), link: "/settings/downstream" },
  { id: "a3", kind: "into_bus", summary: "+3 号 · Kiro Market → Kiro 常驻车", amount: -C(8), created_at: ago(1), link: "/buses/bus_kiro" },
  { id: "a4", kind: "refill", summary: "周末拼车局 · 号少于 3 · 自动触发", amount: null, created_at: ago(1.4), link: "/buses/bus_weekend" },
  { id: "a5", kind: "into_bus", summary: "+5 号 · Kiro CEO → 周末拼车局", amount: -C(12), created_at: ago(2.7), link: "/buses/bus_weekend" },
  { id: "a6", kind: "dead", summary: "cred_…4F2 · Kiro Drop · 存活 42h", amount: null, created_at: ago(3.5), link: "/buses/bus_weekend" },
  { id: "a7", kind: "extract", summary: "+10 号 · Kiro OOO → 待派", amount: -C(25), created_at: ago(4.3), link: "/extract" },
  { id: "a8", kind: "topup", summary: "waffo · 支付宝 · 200 元 → 到账 190", amount: C(190), created_at: ago(9.8), link: "/wallet" },
  { id: "a9", kind: "redeem", summary: "兑换码 KIRO-8Q2P · 邀请奖励", amount: C(50), created_at: ago(12), link: "/wallet" },
];

/* ── Vendor ── */

export const vendorStats: VendorStat[] = [
  { vendor_id: "91kiro", rank: 1, unit_price: C(20), avg_lifespan_seconds: 42 * 3600, effective_cost: 0.48, alive_rate: 98, pulls_today: 12, fallback_count: 0, out_of_stock: false },
  { vendor_id: "kiroceo", rank: 2, unit_price: C(18.5), avg_lifespan_seconds: 36 * 3600, effective_cost: 0.51, alive_rate: 95, pulls_today: 8, fallback_count: 0, out_of_stock: false },
  { vendor_id: "kirooo", rank: 3, unit_price: C(22), avg_lifespan_seconds: 38 * 3600, effective_cost: 0.58, alive_rate: 92, pulls_today: 3, fallback_count: 1, out_of_stock: false },
  { vendor_id: "kirodrop", rank: 4, unit_price: C(15), avg_lifespan_seconds: 22 * 3600, effective_cost: 0.68, alive_rate: 88, pulls_today: 5, fallback_count: 2, out_of_stock: false },
  { vendor_id: "kiroappio", rank: 5, unit_price: C(25), avg_lifespan_seconds: 30 * 3600, effective_cost: 0.83, alive_rate: 85, pulls_today: 0, fallback_count: 0, out_of_stock: false },
  { vendor_id: "kiroappcc", rank: 6, unit_price: 0, avg_lifespan_seconds: 0, effective_cost: 0, alive_rate: 0, pulls_today: 0, fallback_count: 0, out_of_stock: true },
];
...
export const overview: Overview = {
  ...
  buses: {
    ...
    items: buses.map((b) => ({
      id: b.id, name: b.name, alive: b.alive_count, dead: b.dead_count, spend: b.spend_today,
    })),
  },
  extract: {
    count_today: 4, total_credentials: 20, pending: 10, spend: C(49),
    by_destination: [
      { destination: "pending", count: 10 },
      { destination: "into_bus", count: 5 },
      { destination: "push_pool", count: 2 },
      { destination: "handoff", count: 3 },
    ],
  },
};

const SEED = [22, 28, 18, 35, 42, 30, 25, 38, 45, 52, 48, 40, 55, 62, 58, 44, 50, 68, 72, 65, 58, 70, 82, 75, 68, 80, 88, 72, 78, 85];

export function trend(metric: string, days = 30): TrendPoint[] {
  const scale = metric === "credits" ? 1 : metric === "pulls" ? 0.06 : 0.55;
  return Array.from({ length: days }, (_, i) => {
    const d = new Date();
    d.setDate(d.getDate() - (days - 1 - i));
    const v = SEED[i % SEED.length] * scale;
    return {
      date: d.toISOString().slice(0, 10),
      value: metric === "pulls" ? Math.max(1, Math.round(v)) : Math.round(v * 10) / 10,
    };
  });
}
...
export const ledger: LedgerEntry[] = [
  { id: "l1", type: "spend", amount: -C(6), balance_after: C(1245), memo: "提取 key · 2 号 · Kiro Drop", created_at: ago(0.5) },
  { id: "l2", type: "spend", amount: -C(25), balance_after: C(1251), memo: "提取 key · 10 号 · Kiro OOO", created_at: ago(4.3) },
  { id: "l3", type: "spend", amount: -C(18), balance_after: C(1276), memo: "拿走 · 3 号 · Kiro CEO", created_at: ago(26) },
  ...
  { id: "l6", type: "warranty_refund", amount: C(9), balance_after: C(1054), memo: "号 30 分钟内死亡 · 质保退款", created_at: ago(126) },
];
