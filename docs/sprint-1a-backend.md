...
## 计费：服务费 = **号数 × 1 积分**（`decisions §8.33`）

**按号，不是按次** —— 拉 1 个 = 1 积分，拉 10 个 = **10 积分**。

文档里早先写的「一次动作一轮，跟 count 无关」**是错的**（那样拉 10 个只收 1 积分），`00 §3` / `05-api-contract` / `CLAUDE.md` 都已改。

影响 **Iss #5（wallet）· Iss #9（decider 计费）· Iss #10（bus pull）**：

- `pull_round.service_fee_total = purchased × SERVICE_FEE_PER_KEY`
- **单一费率**，不分社群 / 零售 —— 区分那两类是**区域附加费 20%** 的职责（1b 才做）
- **单价做成配置项**，不写死常量（后台可调）
- 多人车：一轮 9 个号 = 9 积分，按各人 `share_pct` **分摊**（不是每人 9）· 1a 全 1 人车，但公式一开始就按分摊写
- **服务端裁定**，客户端传什么都不信

其余待确认项（`§8.32` 里 vendor 附加费 / 区域附加费的具体率）**不挡 1a** —— 1a 只有 91kiro 单家单区，加价栈是 1b 的事。
