...
> ⚠️ **本文已过时 · 状态列不可信（2026-08-14 标记）**
>
> 下面"生产上线/剩余"各列钉在 **2026-08-12** · 之后抢号链、xi8 后台 backfiller、
> 洗脏数据、自适应频次、migration 027–032 **均已部署** · 但本文没更新 —— 这就是
> CLAUDE.md §10.4 说的"状态列腐烂"。**别再照这里的 ❌ 判断有没有上线**。
>
> **看这些替代**：
> - 端点补接分类 + 优先级 → `20-endpoint-classification.md`
> - 抢号链设计 → `16-buy-race.md`
> - 字段/webhook 实测 → `19-fields.md`（§19.2b 是最新实测）
> - **有没有上线 → 看代码 + `git log` + 生产 `docker logs`**
>
> 本文保留仅作**三层框架（A 上游 / B 内部 / C 用户端）的历史参照** · 待拆分后废弃。
...
**用账号密码逐家登进后台 + Playwright 抓文档 + Network 抓隐藏端点**（v1 是探针式 curl · v2 是逆向）。各家端点已并进 `docs/vendors/<家>.md` 的 §2（原 audit 中间产物按 §8.4 未入库）。
