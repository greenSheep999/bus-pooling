...
**乘客视角**：「我要拼车」→ 建车 → 系统给**拼车码** → 分享给朋友 → 朋友输码加入 → 拉号

**发生了什么**：同 A2 · 成员通过**拼车码**显式加入而非匿名撮合。集单 + 拉号 + 记账 + 分摊逻辑与 A2 一致。
...
**乘客视角**：朋友给我**拼车码** → 我在 UI 输码 → 加入 bus

**发生了什么**：`bus.JoinByInvite(code, passenger_id)` → 校验**拼车码** → 同 A4 后续。**阶段**：1c。
...
| A5 · 加入拼车码 bus | (bus.JoinByInvite) + D | 1c |
