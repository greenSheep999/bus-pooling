...
**乘客视角**：账户页点"充值 100 元"→ 扫码支付 → 到账 **95 积分**（不是 100）

**发生了什么**：走时序 B。乘客支付 100 CNY → payment-gateway 扣 5 CNY 通道费 → 我方到账 95 → 乘客积分 +95。**通道费 pass-through**，我方不代垫。**阶段**：1b。
...
**发生了什么**：decider 拉号动作里判 `count == 1` → 在号价基础上加 20% → ledger 落 `single_pull_fee`（我方收入）。

**账单示例**（vendor 号价 20 CNY）：
- 号价 20 · 单次议价 20% × 20 = 4 · 服务费 1 → 小计 25 · 通道费 5% × 25 = 1.25 → **总付 26.25 CNY**
...
**账单示例**（vendor 号价 20 × 3 = 60 CNY）：
- 号价 60 · 单次议价 0 · 服务费 1 → 小计 61 · 通道费 5% × 61 = 3.05 → **总付 64.05 CNY**

**关键**：拼车拉一次即使参与人只想拿 1 个，若合流后 `count_total >= 2`，仍走批量口径（不触发单次议价）。**阶段**：1a。
