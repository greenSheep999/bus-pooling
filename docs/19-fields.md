...
> **不列"已接/覆盖率"** —— 那种数字每次部署就腐烂（旧版实测：说 kiroceo `100%` ·
> 实际 `keys/usage`+`webhook` 没接）。**连接状态只以代码为准**（各家 `adapter.go`
> 实际 HTTP 调用）· **补接分类与优先级看 `20-endpoint-classification.md`**。

| vendor | 官方端点数 | 端点风格 |
|---|---:|---|
| **kiro91** | ~31 | `/api/my/*` · 最详细文档（684 行）|
| **kirooo** | ~32 | `/api/my/*` · **端点最多** |
| **kiroappio** | ~25 | `/api/me/*` · 分页信封 |
| **kiroceo** | ~9 | `/api/my/*` · 最精简 |
| **kiroappcc** | 5 公开 + 15 隐藏 | `/openapi/*`（公开）+ `/api/user/*`（隐藏）· camelCase |
| **kirodrop** | ~8 | `/api/my/*` + `/api/me/*` + `/api/v1/*` 混用 |
| **xi8**（聚合源）| ~25 | `/api/*` + `/api/my/*` |
