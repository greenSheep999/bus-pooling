...
> **⛔ 这份已废（2026-08-14）**：v1/v2/v3/v4 是我一时兴起造的第二套编号 · 跟
> **CLAUDE.md §7 / docs/00 §7 的阶段表（1a/1b/1c/1d/1e）** 双轨冲突 · 导致跟用户对话时
> "vendor 全部做完了？1c 1d 都做完了？" —— 我脑子里数着 v3.5/v4.4 · 用户问的是 1c/1d ·
> 两套编号在一个仓库里就是错的。
>
> **只留一套**：CLAUDE.md §7 阶段表（1a/1b/1c/1d/1e/2a/2b/3a/3b/3c/3d）。
> 讨论进度 / 分派任务 / 写 commit 都用这套。
>
> 我之前那 6 个 v3/v4 commit 的实际归属：
>
> | 我之前叫 | 实际归属（阶段表） |
> |---|---|
> | v3.2 自动补车 Step 2 | **1d**（号死自动补车） |
> | v3.3 vendor 余额切换 | **1d**（决策模型 fallback） |
> | v3.5 陈旧管线告警外发 | 跨阶段运营 · 不属产品阶段 |
> | v4.2 涨价历史端点 | **3a**（数据图表） |
> | v4.3 乘客对账端点 | **3a**（数据图表 · 乘客视角） |
> | v4.4 webhook 落价 | **1d**（价格数据源） |
>
> 所以 1c/1d 都**没做完** —— 1c 一根线都没接（多人拼车 anon+team）· 1d 只有骨架 · 自动模式本身完全没做。
>
> 下面原文保留 · 供历史查证 · **不要照它规划新工作**。
