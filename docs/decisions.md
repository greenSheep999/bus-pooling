...
### 8.9 阶段 1 只做 single bus · anon / team 推迟 ⚠️ **已被后续覆盖**
- **原决定**：阶段 1 只做 `kind=single` · anon/team 推 2a/2b
- **⚠️ 已被覆盖**(见 `00 §7` 现阶段表):**anon 撮合 + 拼车码加入两条路径都在 1c** · 2a 改成"列队策略 / 集单窗口调优"
- **术语更正**:老"team 邀请码组队" = 现在的"**拼车码加入用户车**" · 用户建的车一律带拼车码(single/team 行为完全一致 · `CLAUDE §1.2`)
- **UI**:拼车页 CTA 拼车三项现在 1c 全可点 · 灰化只留未开放的功能
- **参考**：`00 §7` 阶段表 · `decisions §8.42` 四码分离
...
### 8.11 拼车页主入口 = "立即拼车 ▾" 下拉三选一 ✅（1c 更新)
- **决定**：拼车页 hero 下方"我的车"section 右上放**单一主 CTA**"立即拼车 ▾"（紫色 primary），点开下拉 3 项：
  - `建自己的车`（single · 一律带拼车码 · 可分享给朋友）
  - `搭车`（anon · **1c 起可用**）
  - `输拼车码加入`（**1c 起可用** · 输的是**拼车码**·不是好友邀请码 · 见 §8.42 四码分离）
...
### 8.42 四码分离 · 专属邀请码 / 邀请码 / 拼车码 / 优惠码 ✅（2026-08-15 定稿）

**车主拍板**:阶段 1 收官前必须把四种码彻底分开。任何混淆一律作废。

**四码定义** — 内部字段名 / 用户侧名字 / 语义各自独立:

| 用户侧叫 | 内部字段 | 作用 | 补绑? | 改 tier? | 建 referral? |
|---|---|---|---|---|---|
...
- 好友邀请码 = "邀请码" · 用户侧就叫这个名 · 内部实现是 `personal_invite_code`
- UI 不暴露 `retail / community / wholesale` · 也不暴露"社群码 / 批发商码"
- 用户只看到"是否绑定专属邀请码" → 显示"社群成员" chip · 具体档次不暴露
- **只有 wholesale 能看 vendor 真名** · community 也不看

**已废概念**(不许再用):
- ❌ "邀请码组队" · "系统邀请码"(单一 · 内部要拆) · "insider / 同行 / 同行码"
- ❌ "填任何邀请码都免加价 + 看 vendor 真名"(个人邀请码不改 tier · 社群档不看真名)
- ❌ "个人邀请码奖励 N 轮服务费全免"(改为通道费/手续费减免 + 硬上限)
- ❌ "好友邀请码可以注册后补绑"(只有专属邀请码能补绑)
- ❌ "拼车邀请码"(必须叫"拼车码")

**权威口径出处**:
- 阶段 / 版本 → `docs/00-values-and-phases §7`
- tier / 定价 / 减免栈 → `docs/10-pricing §2.1`
...
**⚠️ 当前 schema 已无 `insider`**(2026-08-11 §8.40 命名归一化):三档改成 `retail / community / wholesale` · migration 已把 `insider` 数据迁到 `wholesale`。看当前口径请查 `docs/10-pricing §2.1` 和 `docs/06-db-schema.md § passenger.tier`。
...
### 8.29 邀请制 · 个人码 vs 系统码 + 邀请奖励 ⚠️ **已被 §8.42 四码分离覆盖**

**车主原话**：
> 第一版上线之后立马补充邀请制。个人邀请码、和系统邀请码划分零售和社群的不是一种。邀请人进来可以 N 轮减免 5% 手续费或者服务费，看哪种。

**当前权威口径**(见 `§8.42` 四码分离):
- **专属邀请码**(内部 `system_invite_code`) · `grants_tier` 只能是 `community` / `wholesale` · 老 `insider` 已废
- **个人邀请码**(内部 `personal_invite_code`) · 建 referral · 奖励为**手续费/通道费减免额度**(必须有硬上限 + 单独记 `channel_fee_subsidy` 科目) · **不改被邀请人 tier**
- 老"个人邀请码奖励 N 轮服务费全免"**不是当前阶段 1 收官口径** · 详见 §8.42

**保留本条讨论备查** —— 但**当前实现按 §8.42 走**。
...
- **推翻**：早期设计的独立"策略页"作废
- **参考**：`docs/archive/sprint-1a-frontend.md` #F10 需要更新（已归档）
...
6. ✅ **用户字段命名和语义** · 已回落 `docs/15-scheduling.md`：`RefillWatermark` = 水位线，`RefillMinCount` = 本轮最少拉几个；车级 `DailyRoundLimit` / `DailySpendLimit` 是 deprecated 字段，当前只读乘客全局 limit。
...
### 12.已定 · 决策器边角一次拍板（2026-08-15）

`docs/15-scheduling §5.6` 用户逐个拍板 · 完整决策留 15-scheduling · 本节速览：

- **① 备胎判据 = 数量 AND 价格双条件** · 复用 `RefillMinCount` + 加价格上限过滤 · "先把超价的 vendor 排除掉·再看剩下能下单的家里有没有活号够 min_count 的"
- **② ⏸ 抢号能力 = 占坑排队** · 这是待设计能力，不是当前实现。当前 `stockwatch` 是系统缺货挂单能力：先挂先抢、不预冻结、无 `prebuy_enabled` 字段、无付费优先排序。若要做 Alice/Bob 付费优先，必须另补 migration + API + UI + 扣费/退款测试。
- **③ 排队 10 分钟没抢到自动放弃**(TTL 保留现默认值)
- **④ ⏸ 用户开抢号开关是唯一入口** · 仅适用于未来付费优先能力；当前没有 `prebuy_enabled` 订阅入口，也没有"POST 临时抢一次" API。
- **⑤ usage 见底阈值** · 待 `credential_usage_snapshot` 数据采集上线后按实测定 · 不阻塞

**代码口径**:现有 `stock_watcher.reserved_amount` 恒为 0，挂单时不预冻结；fire 时才走 `decider.Pull` 的完整冻结 / 扣款 / 退款链。未来 prebuy 若要收费，不能复用当前文案，必须重新定义费种和状态机。

**这五条覆盖了 `§12` 六条待补位置里的两条半**:位置 3(多 vendor 判据)由 ①解；位置 6(字段命名)已回落 `15-scheduling`；位置 5 的"挂单优先"目前只覆盖系统缺货挂单，不覆盖付费 prebuy。剩下位置 1 / 2 / 5 的付费化部分仍是决策器代码改造点；位置 4 已在 12.已定 定过。
