...
| GET | `/api/me/buses/{bus_id}/credentials` | 该 bus 的号列表（含存活状态） | 1a |
...
| GET | `/api/me/credentials/{id}` | 单号详情（含 group / 存活状态 / 用量） |

**这个是**"我名下**在 housepool 里**的所有号"的入口。`handoff` 出去的号不在这里（已离开 housepool）。
