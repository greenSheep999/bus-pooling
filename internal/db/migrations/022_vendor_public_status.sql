...
--
-- 之前的 vendor_probe 只记我方账户的库存视图（Stock 端点）。有些 vendor（kirooo /
-- kirodrop / kiroappio）还有 /api/status 端点，返 fleet-wide 累计状态：
-- keys_active / keys_dead / keys_stock / generating / uptime_seconds / started_at
--
-- 落库策略：跟 Stock 探测同一行 · 都是 poller 一次探测的产物。
-- 没有 /api/status 端点的 vendor（kiroceo / 91kiro / kiroappcc）这些字段为 NULL。
--
...
ALTER TABLE vendor_probe ADD COLUMN ps_keys_alive      INTEGER;    -- kirooo 特有 · active + suspect
...
ALTER TABLE vendor_probe ADD COLUMN ps_keys_suspect    INTEGER;    -- kirooo 特有 · 探测异常但没判死
